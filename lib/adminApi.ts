// ── Admin API ─────────────────────────────────────────────────────────────────
// Appelle les Next.js API Routes /api/admin/* — plus de proxy DreamHost.

import { Code, Article, PaginatedResponse, User } from './api'
...
export interface AdminArticle {
  id: string; code_id: string; number: string; number_int: number | null
  slug: string; status: string; view_count: number; comment_count: number
  created_at: string; code?: Pick<Code, 'id' | 'slug' | 'title_ar'>
}

export interface UserStats {
  total: number; active: number; suspended: number
  by_role: Record<string, number>; new_today: number; new_week: number
}

export interface ArticleStats {
  total: number; in_force: number; abrogated: number; amended: number
  by_code: { name: string; count: number }[]
}

export interface AdminComment {
  id: string; article_id: string; content_ar: string; type: string; status: string
  rejection_reason: string | null; upvotes: number; created_at: string
  article?: { id: string; number: string; slug: string; code?: Pick<Code, 'id' | 'slug' | 'title_ar'> }
  author?: Pick<User, 'id' | 'full_name' | 'email'>
}

export interface AdminNote {
  id: string; content_ar: string; created_at: string
  author?: { full_name: string | null }
}

export interface AdminPdf {
  id: string; code_id: string | null; title_ar: string; title_fr: string | null
  original_filename: string; stored_filename: string; file_size: number
  document_type: string; status: 'pending' | 'processing' | 'imported' | 'failed'
  articles_extracted: number | null; extraction_log: string | null
  source_url: string | null; is_public: boolean; created_at: string
  code?: Pick<Code, 'id' | 'slug' | 'title_ar'> | null
  uploader?: { id: string; full_name: string | null } | null
}

export interface ExtractResult {
  message: string; articles_detected: number; articles_imported: number; log: string
}

export interface CodeType {
  id: number; slug: string; name_ar: string; name_fr: string | null
  color: string; sort_order: number; codes_count?: number
}
...
export const adminUsers = {
  ...
  update:  (id: string, data: { role?: string; status?: string; full_name?: string }) =>
    adminFetch<{ message: string; user: AdminUser }>(`/users/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  destroy: (id: string) =>
...
export const adminArticles = {
  ...
  bulkUpdateStatus: (ids: string[], status: string) =>
    adminFetch<{ message: string }>('/articles/bulk-status', { method: 'POST', body: JSON.stringify({ ids, status }) }),
  bulkDestroy: (ids: string[]) =>
    adminFetch<{ message: string }>('/articles/bulk-delete', { method: 'POST', body: JSON.stringify({ ids }) }),
}
...
export const adminComments = {
  ...
  update: (id: string, data: { status?: string; rejection_reason?: string }) =>
    adminFetch<{ message: string; comment: AdminComment }>(`/comments/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  destroy: (id: string) =>
    adminFetch<{ message: string }>(`/comments/${id}`, { method: 'DELETE' }),
}

export const adminNotes = {
  list:    (articleId: string) => adminFetch<AdminNote[]>(`/articles/${articleId}/notes`),
  create:  (articleId: string, content_ar: string) =>
    adminFetch<{ message: string; note: AdminNote }>(`/articles/${articleId}/notes`, { method: 'POST', body: JSON.stringify({ content_ar }) }),
  destroy: (noteId: string) =>
    adminFetch<{ message: string }>(`/articles/${noteId}/notes`, { method: 'DELETE' }),
}

export const adminPdfs = {
  list:    (page = 1) => adminFetch<PaginatedResponse<AdminPdf>>(`/pdfs?page=${page}`),
  upload:  (formData: FormData) =>
    adminFetch<{ message: string; pdf: AdminPdf; download_url: string }>('/pdfs', { method: 'POST', body: formData }),
  get:     (id: string) => adminFetch<{ pdf: AdminPdf; download_url: string; file_size: string }>(`/pdfs/${id}`),
  update:  (id: string, data: { title_ar?: string; title_fr?: string; code_id?: string; is_public?: boolean }) =>
    adminFetch<{ message: string; pdf: AdminPdf }>(`/pdfs/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  extract: (id: string) => adminFetch<ExtractResult>(`/pdfs/${id}/extract`, { method: 'POST' }),
  preview: (id: string) =>
...
export const adminCodeTypes = {
  list:    () => adminFetch<CodeType[]>('/code-types'),
