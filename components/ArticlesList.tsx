    ...
    <div>
      ...
      <div className="space-y-3">
        {articles.map((article) => (
          <Link
            key={article.id}
            href={`/codes/${slug}/articles/${encodeURIComponent(article.slug)}`}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                {article.section?.title_ar && (
                  <p className="text-xs text-slate-400 mb-1">{article.section.title_ar}</p>
                )}
                <p className="text-slate-600 text-sm leading-relaxed text-arabic">
                  {truncate(article.content_ar)}
                </p>
              </div>
