...
export default function LoginPage() {
  ...
  /* Redirection vers le backend Google OAuth */
  const handleGoogle = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-slate-50
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-100 p-8">
        ...
        {process.env.NODE_ENV !== 'production' && (
          <div className="mt-4 p-3 bg-amber-50 rounded-xl border border-amber-100">
            <p className="text-xs text-amber-700 font-medium mb-1">للتجربة السريعة (dev) :</p>
            <button
              type="button"
              onClick={() => { setEmail('admin@almudawwana.ma'); setPassword('Admin@2024!'); }}
              className="text-xs text-amber-600 hover:underline font-mono"
            >
              admin@almudawwana.ma / Admin@2024!
            </button>
          </div>
        )}
      ...
      {/* Disclaimer sous la carte */}
      <p className="mt-5 text-xs text-slate-400 text-center max-w-sm leading-relaxed">
        المدوّنة للمعلومات العامة فقط — لا تغني عن النصوص الرسمية في{' '}
