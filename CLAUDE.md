...
# PWA & OG image asset generation (root-level Node scripts, require sharp)
node generate-pwa-assets.mjs   # Regenerate all PWA icons + screenshots from logo SVG
node generate-og-image.mjs     # Regenerate public/og-image.png (1200×630)
...
- **BASE_URL:** `https://modawana.app` — hardcoded in `app/layout.tsx`, `app/page.tsx`, `app/sitemap.ts`, `components/JsonLd.tsx`, and all pages with `generateMetadata`
- **OG image:** `public/og-image.png` (1200×630, gradient blue, Arabic title) — regenerate with `node generate-og-image.mjs`
...
**Viewport:** `maximumScale: 5` — do NOT set back to 1 (WCAG accessibility requirement, also fixes PWABuilder warning).
...
- **SW registration:** Two mechanisms — inline `<Script id="sw-register" strategy="afterInteractive">` in `app/layout.tsx` (early boot) + `components/ServiceWorkerRegister.tsx` (React lifecycle). Both register `/sw.js` at scope `/`.
- **PWA manifest** (`public/manifest.json`): includes `id`, `lang`, `dir`, `display_override`, `prefer_related_applications: false`, and `screenshots`. Validated by PWABuilder (0 errors). To add IARC rating: set `"iarc_rating_id"` field.
- **PWA icons:** Generated by `node generate-pwa-assets.mjs` from the logo SVG (sharp). Outputs: `icon-192x192.png`, `icon-512x512.png`, `icon-maskable.png`, `apple-touch-icon.png`, `screenshot-wide.png` (1280×720), `screenshot-narrow.png` (540×960).
...
## Play Store / PWABuilder (pending)

- PWABuilder manifest: validated (0 errors, 1 advisory about SW scope — expected)
- **IARC rating:** obtain at iarcratings.com → add `"iarc_rating_id": "..."` to `public/manifest.json`
- **assetlinks.json:** create at `public/.well-known/assetlinks.json` with Play Store keystore SHA-256
- **Google Play Console:** manual publish after generating PWABuilder package
- **Google Search Console:** manually submit `https://modawana.app/sitemap.xml`
